...
-r: Equivalent to -o ro
-w: Equivalent to -o rw
-L: Mounts a partition with a specific label.
-U: Mounts the file system with the specified UUID. -L and -U only make sense when there is a /proc/partitions file available.
...
-o dev, -o nodev: Allows/disallows using device files as file systems. -o exec, -o noexec: Allows/disallows executing binary files on the file system.
-o suid, -o nosuid: Allows/disallows setuid and setgid bits to take effect.
-o user, -o nouser: Allows/disallows ordinary users to mount (with nouser, only root can mount).
