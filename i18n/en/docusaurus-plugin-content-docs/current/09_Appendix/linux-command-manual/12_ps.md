...
- `X`: Register format.
- `-y`: Do not display flags, show RSS in place of address (only with -l).
...
- `S`: represents the STAT (state) of this program, and the meanings of different STATs will be explained below.
  - `S`: Interruptible sleep - the process is in an interruptible sleep state.
...
List all the currently running programs in memory (excerpt):
...
- [top](./18_top.md)
- [Viewing System Logs](../../02_System_configuration/15_system_log.md)