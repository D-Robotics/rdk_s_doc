...
Many issues can be resolved by updating the system. For related download resources, see: [Download Resources](../01_Quick_start/download.md)
...
For certified accessories and purchase links, see the [RDK S100 Certified Accessories List](/Advanced_development/hardware_development/accessory)
...
**Important notes:**
* **Version upgrade:** RDK OS 1.x **cannot** be upgraded directly to 2.x or newer via `apt`. To upgrade, you must re-flash a new system image and [install the operating system](/Quick_start/install_os/rdk_s100/instruction) again.
...
3. **Switch from beta to release repository**
   As of 2025-07-14, the RDK S100 release repository was not yet published.
...
1.  **Via `/etc/rc.local` (legacy):**
    Edit this file (create it or enable `rc-local.service` from a template if missing) and add commands before `exit 0`. Ensure the script is executable.
    ```bash
    #!/bin/bash -e
    #
    # rc.local
...
* **Superuser (root):** username `root`, password `root`
  (Default credentials may vary by image version and source; check the release notes for your image.)
...
2.  **Mount the NTFS partition:**
    After installation, use `mount`. The system usually selects the `ntfs-3g` driver automatically for read/write access.
...
**Note:** Steps vary by RDK model and OS version. See official ADB documentation for your board. Bootloader update guides may also mention ADB prerequisites.
Reference (may focus on bootloader updates but relates to ADB): [Bootloader image update](https://developer.d-robotics.cc/forumDetail/88859074455714818) (see ADB-specific documentation when available).
...
2.  **SFTP (SSH File Transfer Protocol) — over SSH:**
    Clients such as FileZilla and WinSCP support SFTP with a graphical UI. Select SFTP and use SSH credentials and the board IP.

3.  **USB storage (flash drive/external disk):**
...
4.  **ADB (Android Debug Bridge) — if configured:**
...
5.  **Network shares (Samba, NFS):**
    Configure Samba or NFS on the board to share directories on the LAN. Setup is more involved.

6.  **Python HTTP server (quick small-file sharing):**
    To let a PC download files from a directory on the board, start a simple HTTP server there:
    ```bash
...
2.  **Cable:** Use a good HDMI cable with firm connections; try another cable.
...
4.  **Power:** Severe undervoltage can prevent proper display subsystem initialization.
5.  **Hardware:** Rarely, a faulty board HDMI port or monitor.

### Q17: My HDMI monitor is unsupported. How do I capture EDID for technical support?
**A:** If HDMI output fails or is abnormal, support may need EDID (Extended Display Identification Data) to diagnose issues or add compatibility. EDID describes monitor capabilities, resolutions, and timings.
...
2.  **Dedicated EDID hardware/software:** Some test tools or EDID programmers can read and save EDID directly.
3.  **Read on a PC:** If the monitor works on a Linux or Windows PC, capture EDID there.
    * On Linux: `xrandr --props` or `get-edid | parse-edid`.
    * On Windows: tools such as MonitorInfoView (NirSoft) or Phoenix EDID Designer.

Provide the EDID data (binary file or hex text) to technical support.
For EDID capture on D-Robotics RDK platforms, see: [How to provide EDID for unsupported monitors](https://developer.d-robotics.cc/forumDetail/235046352323895808)
...
2.  **Contact between card and slot:**
...
5.  **Cross-check:**
If possible, flash on another PC with the same card/reader, or try different card/reader combinations on the same PC.
...
2.  **Airflow:** Avoid enclosed or poorly ventilated locations.
...
5.  **Power:** Unstable supply can cause abnormal operation that affects thermals.
...
2.  **pip from system paths (if no `.whl`):**
...
6.  **Build from source (if provided):**
...
1.  **Generate a signing key pair:**
    Use `openssl` to create public and private keys.
    ```bash
...
2.  **Sign the kernel module:**
    Use the kernel `scripts/sign-file` script with your keys to sign the `.ko` file.
    ```bash
...
3.  **Enroll the public key in MOK (Machine Owner Key):**
    Import `MOK.der` with `mokutil` so the kernel trusts your signature.
    ```bash
    sudo mokutil --import MOK.der
    ```
    You will set a one-time password. **Remember it.**

4.  **Reboot and confirm in MOK Manager:**
    After reboot, in the blue MOK Manager (Shim) screen, choose "Enroll MOK" and enter the one-time password.

5.  **Load the signed module:**
    After enrollment, loading the `.ko` file should succeed without signature errors.
...
1.  **Increase swap space:**
    Swap uses disk as virtual memory when RAM is exhausted. It helps OOM but is much slower than RAM.
...
2.  **Reduce build parallelism:**
    Parallel builds (especially C++) use more memory.
...
4.  **Stop unnecessary services and apps:**
    Close GUI, browsers, and other services before heavy builds to free RAM.

5.  **Use a more capable build machine:**
    Cross-compile on a higher-memory x86 host when possible; on-board builds with limited RAM should prefer cross-compilation.

6.  **Incremental or modular builds:**
    Build subprojects or modules separately when the build system allows.
...
4.  **Provide complete reproduction details:** When posting to the community or support, include:
    ...
    * **Troubleshooting you already tried and the results.**
    Complete information helps others diagnose faster.
...
3.  **Embedded development Samples:**
...
1.  **Cross-compiling plain Linux C/C++ applications:**
    ...
    * **CMake cross-build:** Create a toolchain file (e.g., `aarch64-rdk.cmake`) specifying:
        ...
        * `CMAKE_FIND_ROOT_PATH` for libraries and headers.
        Configure with `-DCMAKE_TOOLCHAIN_FILE=/path/to/your/aarch64-rdk.cmake`.
    * **Official manuals:** See the user manual or SDK guide for Linux application development and cross-compile setup.

2.  **Cross-compiling ROS/TROS packages:**
...
**A:** IMX219-class MIPI camera modules connect via a 24-pin FPC (flexible flat cable).
