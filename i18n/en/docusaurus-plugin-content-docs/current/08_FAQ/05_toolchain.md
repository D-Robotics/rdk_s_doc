...
For toolchain issues, we recommend using the latest version first. For related download resources, see: [Download Resources](../01_Quick_start/download.md)
...
- interpolation: Interpolation method. Only effective when mode is opencv. Default: empty. Valid values: (OpenCV interpolation methods).
...
Divides target size by the maximum of input image width or height as a coefficient, multiplies original width and height by this coefficient to resize the image.
Then based on the new image size, divides by size_divisor, rounds up, multiplies by size_divisor to get new width and height, and generates a new image.
