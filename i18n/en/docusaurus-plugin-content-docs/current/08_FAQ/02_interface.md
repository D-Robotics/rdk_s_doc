...
3.  **Serial terminal software configuration:**
...
5.  **Refer to official documentation:** For detailed network configuration steps and troubleshooting methods, refer to the "Network Configuration" section in the official documentation.
    ...
    [RDK S100 Network Configuration](/System_configuration/network_bluetooth)
    </DocScope>
...
* **Message `Connection timed out`:**
    ...
    * **Reference:** [SSH Login](../01_Quick_start/remote_login.md) section (Please replace the link with the actual valid documentation path).
...
3.  **Driver and firmware:** Ensure the Wi-Fi module driver and firmware on the development board are up to date or stable (usually provided through system updates).
4.  **Network load:** Check whether other devices on the LAN are consuming a large amount of bandwidth.
5.  **2.4GHz vs 5GHz:** If both your development board and router support 5GHz Wi-Fi, try connecting to the 5GHz band. It usually has less interference and higher speed (but weaker wall penetration than 2.4GHz).
...
3.  **Hardware issue:** In rare cases, the wireless module itself may be faulty.
4.  **System image or configuration issue:** Ensure the system image you are using supports the onboard wireless module and that the related kernel configuration is enabled.
...
3.  **Avoid Micro USB port conflicts (for specific boards):**
...
1.  **Incorrect camera connection:**
    ...
    * **Reference documentation:** Carefully review the "MIPI Camera" connection section in the hardware manual or quick start guide for your RDK board model to confirm connection details.
        <DocScope products="RDK S100">
        [RDK S100 MIPI Camera](/01_Quick_start/01_hardware_introduction/01_rdk_s100/02_rdk_s100_camera_expansion_board/01_rdk_s100_camera_expansion_board.md)
        </DocScope>
2.  **Do not hot-plug:**
...
4.  **Camera power supply or clock issue:** Ensure the camera module receives the correct power supply and that the MIPI clock signal is normal.
5.  **Camera module or FPC cable damage:** If you have checked all the factors above and still cannot detect the device, the camera module itself or the FPC cable may be physically damaged.
6.  **Device tree configuration:** Ensure the Linux kernel device tree correctly configures the MIPI interface and the driver information for the connected camera model.
...
* **Query methods:**
    ...
    3.  **`srpi-config` tool:** In some RDK system versions, the `srpi-config` tool may allow you to view or configure HDMI output resolution.
