...
1.  **The stream is missing PPS and SPS parameter information:**
    ...
    * **Solution:**
        * If you use `ffmpeg` to stream from a video file (such as `.mp4` or `.avi`), it is recommended to add the `-bsf:v h264_mp4toannexb` (H.264 Bitstream Filter: MP4 to Annex B) option to the command (note: in newer versions of ffmpeg, `-vbsf` has been replaced by `-bsf:v`). This filter automatically adds `PPS` and `SPS` information to the stream.
            **`ffmpeg` streaming command example:**
            ```
...
1.  **View recognized sound cards and their indices:**
    Use the following command to list all recognized sound cards in the system along with their indices and names:
    ```bash
...
2.  **Use `amixer` or `tinymix` to operate on a specific sound card:**
...
1. Modify the PulseAudio configuration file: `/etc/pulse/default.pa`
    ...
    :::tip
    In the configuration above, `X` in `device=hw:X,Y` represents the sound card number and `Y` represents the device number. Configure them according to your actual requirements. For how to confirm sound card/device numbers, see [Control Commands](../03_Demos/01_peripheral/03_audio.md#control-commands).
    :::
