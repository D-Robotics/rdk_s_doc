...
The globaltime attribute in the dts indicates the default global timeline used by the system. RTC time is used by default. The mapping is as follows:
...
```
2. On S600, the default function of the PIN is already PPS_OUT FUNC, so no modification is required.
...
### Introduction to Acore Eth PPS {#Acore\_Eth\_PPS}
