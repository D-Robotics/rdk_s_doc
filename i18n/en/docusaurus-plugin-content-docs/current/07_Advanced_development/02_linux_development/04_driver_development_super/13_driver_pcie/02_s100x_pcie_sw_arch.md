...
3. After system resume, the following process must be followed:
   - Load the EP driver
   - Load the RC driver
   - Start PCIe applications
4. If the link signal is unstable or a link down occurs due to a peer restart, restart both RC and EP to ensure PCIe functionality and system stability.
5. Do not load PCIe-related drivers when the PCIe function is not in use.
6. Before system restart, the following process must be followed:
   - Stop PCIe applications on both ends
   - Unload the RC driver
   - Unload the EP driver
