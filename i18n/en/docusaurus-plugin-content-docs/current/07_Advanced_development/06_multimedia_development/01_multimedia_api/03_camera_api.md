...
sidebar_position: 3
title: "Camera API"
description: RDK S100/S600 Camera API
---
...
Camera is the capture entry of the RDK multimedia pipeline (board header `hb_camera_interface.h`, functions `hbn_camera_*`). It wraps sensor config, channel init and VIN binding, forming a vflow with HBN vnodes (VIN/ISP/PYM).

## Abstraction

- `camera_config_t`: sensor config (mipi/gmsl, resolution, framerate, etc.).
- `camera_handle_t`: camera handle, used by subsequent APIs.
- Camera binds to VIN via `hbn_camera_attach_to_vin`; frames then flow into the vflow.

## Call flow

1. `hbn_camera_create` creates a camera handle from `camera_config_t`.
2. `hbn_camera_attach_to_vin` binds to a VIN vnode (initializes hardware when no deserializer).
3. `hbn_camera_set_*` / `get_*` configure and query parameters.
4. Frames flow automatically once the vflow starts; `hbn_camera_destroy` destroys the handle.
