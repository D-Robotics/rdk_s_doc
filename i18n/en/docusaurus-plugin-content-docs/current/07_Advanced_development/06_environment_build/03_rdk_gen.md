...
- Build modes: online build (`pack_image.sh`), offline build (`pack_image.sh -l`), and build-environment-only setup (`pack_image.sh -p`).
- Storage media: eMMC / UFS / NVMe (NVMe boot requires RDK S100 V0P6 or later, RDK S600 V1P0 or later).
- Products supported: RDK S100, RDK S600 (each corresponds to a different conf file; see below for how to modify them). For usage instructions, refer to the README.md of the rdk_gen repository.
...
- NVMe boot is supported only on RDK S100 V0P6, RDK S600 V1P0, and later versions. The [D13:D12] DIP switches on the board can be set to [1:0].
...
- After modifying the conf file, recompile the hobot-miniboot, kernel and disk images following "Build Process After Modifying the conf File", and add `-l` to use local deb packages.
- Build artifact paths: `out/product/deb_packages/` (deb packages), `out/product/img_packages/` (system images).

## FAQ

### The build used remote packages instead of locally modified ones

**Cause**: `pack_image.sh` was run without the `-l` option, so deb packages were fetched from remote by default.

**Solution**: use `sudo ./pack_image.sh -l` to ensure the packages under the local `out/product/deb_packages/` are used.

### NVMe boot failure

**Cause**: the board version does not support NVMe boot, or a miniboot_flash image that does not match the NVMe mode was used.

**Solution**: confirm the board is RDK S100 V0P6 or later, or RDK S600 V1P0 or later, and that the miniboot_flash image is compiled for the NVMe mode.
