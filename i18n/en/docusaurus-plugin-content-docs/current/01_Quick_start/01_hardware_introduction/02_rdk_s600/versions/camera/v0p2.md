---
unlisted: true
slug: /01_Quick_start/01_hardware_introduction/02_rdk_s600/versions/camera/v0p2
---

# Camera Expansion Board V0P2
<img src="https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/hardware_interface/image-rdk_s600_camera_expansion_board_v0p2.png" alt="Camera Expansion Board V0P2" style={{ width: '90%', maxWidth: '980px', height: 'auto', display: 'block', margin: '0 auto' }} />
...
The Camera Expansion Board is equipped with an external 12V power input interface for GMSL cameras. When the total current demand at 12V for all GMSL cameras connected to this expansion board exceeds 2A, the GMSL cameras must be powered via this DC power jack.
...
The Camera Expansion Board integrates 2 MAX96712 deserializer chips, each supporting 4 GMSL2 cameras for a total of 8 GMSL2 cameras; it can provide 12V power to the GMSL cameras via coaxial cables.
...
1. When the 12V power current demand of the GMSL cameras is within 2A, an external 12V adapter is not required; the 12V power is then provided by the RDK S600. If the current demand exceeds 2A, an external 12V adapter must be connected to ensure a stable power supply to the GMSL camera modules.
