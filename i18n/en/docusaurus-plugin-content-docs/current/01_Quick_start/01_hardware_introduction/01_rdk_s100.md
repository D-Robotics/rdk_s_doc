...
<img src="https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/hardware_interface/image-rdk_s100_architecture_diagram.png" alt="Topology Diagram" style={{ width: '100%', maxWidth: '980px', height: 'auto', display: 'block', margin: '0 auto' }} />

### Interface Diagram

<img src="https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/hardware_interface/image-rdk_100_mainboard_interface.png" alt="Interface Diagram" style={{ width: '100%', maxWidth: '980px', height: 'auto', display: 'block', margin: '0 auto' }} />
...
The RDK S100 main board includes an RTC external battery connector (J3) for connecting an RTC backup battery, which keeps the real-time clock running after the main board loses power. The onboard RTC chip is YSN8130E, communicating via I2C4 with I2C address `0x32` (excluding the read/write bit).
...
1. A 3V battery is recommended. When connecting an external battery to the connector, the battery voltage is recommended to be within 2.5-3.5V. Rechargeable batteries must withstand at least 3.3V.
2. For rechargeable batteries, the RTC charging function can be enabled as needed; for non-rechargeable batteries, do not enable the charging function.
3. When the system is powered normally, the RTC is preferentially powered by the board-side VDD; after the board loses power, it automatically switches to battery (VBAT) power.
4. After both the main board and RTC (VBAT) lose power and power is restored, the software must be configured as follows:
    - Write the related registers via I2C to disable the FOUT function, to prevent additional power consumption when powered by the battery, which would affect battery life.
    - If using a rechargeable battery (such as Waveshare ML2032), re-enable charging; for non-rechargeable batteries (such as Waveshare CR2032), do not enable the charging function.
...
- USB download interface. The download interface provided by the RDK S100 development board is used for firmware download. For details, please refer to [1.2 System Flashing](../03_install_os_and_setup/01_instruction.md)
