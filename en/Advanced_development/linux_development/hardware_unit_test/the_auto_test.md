...
URL: https://developer.d-robotics.cc/rdk_s_doc/en/Advanced_development/linux_development/hardware_unit_test/the_auto_test
...
1. **Edit the configuration file:** Open and edit the `config/config.ini` file, adjusting the `Status` value for each test item as needed:

- Set to `enabled` : activate the test item.
- Set to `disabled` : deactivate the test item.
2. Set to `enabled` : activate the test item.
3. Set to `disabled` : deactivate the test item.
4. **Modify test parameters:** Adjust the command and parameters in `ExecStart` according to actual requirements. For example:
...
5. Modify test duration (e.g., `-t 24h` means 24 hours of testing).
6. Adjust device paths (e.g., `/dev/ttyS2` or `/dev/spidev0.0` ).
7. Tune other test options (e.g., loop count `-c` or baud rate `-b` ).
8. All current test scripts support the `-h` option, allowing users to consult help information for parameter adjustments.
...
1. **Moving the directory:** Logs are saved by default under the `chip_base_test` directory. Users can simply move the entire `chip_base_test` directory to a new location, for example:
...
```
2. **Customizing the log output path** Driver unit test scripts support the `-o` option to customize the log directory. Users can edit the `config.ini` file and add the `-o` option to the `ExecStart` parameter of the corresponding test item. For example:
