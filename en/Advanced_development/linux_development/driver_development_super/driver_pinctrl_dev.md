...
URL: https://developer.d-robotics.cc/rdk_s_doc/en/Advanced_development/linux_development/driver_development_super/driver_pinctrl_dev
...
The pinctrl-hobot.c file contains the main pinctrl logic code shared by S100/S600, registers the device with the pinctrl subsystem, and provides the initialization entry for SoC data downstream. The pinctrl-hobot-s100.c file contains the pin, pin group, function definitions, and device matching information for the S100.
...
The pinctrl-hobot.c file contains the main pinctrl logic code shared by S100/S600. The pinctrl-hobot-s600.c file contains the pin, pin group, function definitions, and device matching information for the S600 ( `compatible = "drobot,s600-pinctrl"` ).
...
You can query the group and function for each pin through the pinmux-functions node, and set the pinmux through the pinmux-select node with parameters group name and func name. The following command uses `<pinctrl_dev>` as `39ff5000.pinctrl-peri` as an example.
