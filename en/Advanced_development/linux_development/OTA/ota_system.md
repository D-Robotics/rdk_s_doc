...
URL: https://developer.d-robotics.cc/rdk_s_doc/en/Advanced_development/linux_development/OTA/ota_system
...
| Persistent Partition | Parameter partition: Generally stores configuration files and parameters that need to be loaded during system runtime, such as ubootenv in the partition table.

User partition: Refers to partitions unrelated to system startup. Generally mounted only after system startup, such as the userdata partition. | Single partitions typically have no image, partition data needs long-term preservation, and OTA upgrade is not supported. | ubootenv, veeprom, userdata 
...
```
5. Create a new miniboot deb package
```bash
./mk_debs.sh hobot-miniboot
```
6. Recompile the local image
```bash
sudo ./pack_image.sh -l
```
...
4. Partitions on flash do not support differential upgrades.
5. The boot partition has write behavior and does not support differential upgrades.
6. Only supported when the image package size is greater than 10M.
7. If a partition requiring differential upgrade needs to be mounted, it must be mounted in read-only mode with the noload option added; otherwise, MD5 verification may fail.
8. Creating a Differential Upgrade Package
...
```
9. Differential image upgrade relies on the already flashed old image package. Therefore, when planning to use differential upgrade, **be sure to properly save the old image package to avoid loss or damage.**
...
Dijia uses the Android AB mechanism to implement the AB system. The following information introduces some principles of this mechanism. For detailed principles, please refer to: [https://source.android.google.cn/docs/core/ota](https://source.android.google.cn/docs/core/ota)
...
The diagram below illustrates how A/B slots are switched during normal boot (this process applies regardless of whether an OTA update is performed): ![boot_slot_select](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/02_linux_development/image/ota/boot_slot_select-en.jpg)
...
In the S100 reference implementation, after the OTA update completes and the system reboots, the kernel triggers the systemd OTA service to perform a reboot check, thereby completing the full OTA process (essentially executing `ota_tool -b` ). ![ota_boot_check_state](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/02_linux_development/image/ota/ota_boot_check_state-en.jpg)
...
| Output Parameters | N/A 
| Return Value | 0: Success;
-OTAERR_NOTINIT: Currently not initialized, cannot call de-initialization 
...
| Interface Type | C function interface 
| Input Parameters | `image_name` : Absolute path to the upgrade package. Supports passing multiple different types of packages simultaneously, separated by semicolons.
`owner` : The process initiating this upgrade, defined by the `enum ota_update_owner` . 
| Output Parameters | N/A 
| Return Value | 0: Success;
-OTAERR_NULLPOINTER: `image_name` is a null pointer;
-OTAERR_RANGE: `owner` value is out of defined range;
-OTAERR_NOTINIT: Dynamic library not initialized;
-OTAERR_REPEAT: Another process is currently performing an upgrade;
...
| Input Parameters | N/A 
| Output Parameters | N/A 
| Return Value | -OTAERR_NOTINIT: Dynamic library not initialized
`OTA_UPGRADE_NOT_START` : Upgrade has not started;
`OTA_UPGRADE_IN_PROGRESS` : Upgrade in progress;
`OTA_UPGRADE_SUCCESS` : Upgrade finished, successful;
`OTA_UPGRADE_FAILED` : Upgrade finished, failed. 
...
| Input Parameters | N/A 
| Output Parameters | N/A 
| Return Value | 0 ~ 100: Current upgrade progress (%);
-OTAERR_NOTINIT: Dynamic library not initialized. 
...
| Interface Type | C function interface 
| Input Parameters | N/A 
| Output Parameters | `partition` : Pointer to a variable receiving the current partition.
`*partition=0` : Currently in A partition, `*partition=1` : Currently in B partition. 
| Return Value | 0: Success
...
| Return Value | 0: Success
-OTAERR_NOTINIT: Dynamic library not initialized
-OTAERR_RANGE: `partition` value is out of range (must be 0 or 1)
-OTAERR_IO: I/O error, please retry
-OTAERR_STAGE: Operation not supported in the current upgrade stage (e.g., during upgrade or after failure). 
...
| Return Value | `OTA_SUCCESS` : Success
-OTAERR_NOTINIT: Not initialized
-OTAERR_NULLPOINTER: `owner` is a null pointer
-OTAERR_IO: I/O error, please retry. 
...
| Return Value | 0: Success
-OTAERR_IO: I/O error, please retry
-OTAERR_STAGE: No upgrade was performed
-OTAERR_IMAGE_WRITE: Image write failed
-OTAERR_BOOT_FAILED: New image boot failed or partition switch did not occur. 
