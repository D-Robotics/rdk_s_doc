...
URL: https://developer.d-robotics.cc/rdk_s_doc/en/Advanced_development/vdsp_development
...
Load the DSP program through the libvdsp.so shared library to load, start, stop, reset, and query DSP status. For API details, see VDSP Boot/Stop Control APIs .
...
Note that VDSP FW shares a serial port with other modules such as BL31 and OP-TEE. Excessive VDSP FW logging may block log output from those modules and trigger the watchdog. VDSP FW logs and kernel logs are both output to the serial port and may interfere with each other. Users can reduce the kernel log level to avoid interference:
...
Do not register exception handlers on VQ8 for exception causes 4 (debug exception), 5 (SYSCALL exception), or 8 (coprocessor exception); these are reserved for system use. Note: causes 9~15 are reserved and should also be skipped.

Offline debugging procedure: When VDSP coredumps, Acore writes all potentially used VDSP memory spaces (iram/dram0/dram1/reserved ddr) to the file system at the following path:
...
![VDSP development diagram 2](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/image_2.png)The error log is shown below. Error address 0x0 indicates access to a disallowed address:

![VDSP development diagram 3](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/image_3.png)Current VDSP address attribute configuration includes three parts:
...
tip
If the XOS tool cannot find the toolchain during debugging, first confirm that the XOS build environment is set up. If it is, configure the environment variable, for example: `export PATH=$PATH:[*]/XtensaTools/bin`
...
![VDSP development diagram 5](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/image_5.png)Use libidma-os or libidma-debug-os because we use XOS.
...
Modify the SIM xmm file at xtensa/xtensa/XtDevTools/install/builds/RI-2023.11-win32/Vision_Q8/xtensa-elf/lib/sim/memmap.xmm. Increase the size of the overflowing segment, then open cmd in Xplorer:

![VDSP development diagram 6](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/image_6.png)Go to the xtensa-elf/lib path:

![VDSP development diagram 7](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/image_7.png)Run xt-genldscripts -b sim. The message below indicates success:

![VDSP development diagram 8](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/image_8.png)Rebuild the VDSP project and run SIM soft simulation again.
...
![VDSP development diagram 9](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/image_9.png)
#### Windows-built Image Reports dcore0_rpmsg_op Server Not Started

Manually add CONFIG_TEST_CASE in Build Properties:

![VDSP development diagram 10](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/image_10.png)
...
![VDSP sample diagram 1](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/vdsp1.png)
...
![VDSP sample diagram 2](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/vdsp2.png)
#### VDSP-side Development Flow

On the VDSP side, initialize the runtime environment and start related services (VDSP acts as the server and can start multiple one-to-one services), receive and reply to client messages through RPMSG, and start additional threads for application development.

![VDSP sample diagram 3](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/vdsp3.png)
#### Sample Flow Description:

The interaction flow between the ARM side and VDSP is shown below:

![VDSP sample diagram 4](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/07_vdsp_development/vdsp4.png)ARM side:
...
#### RPMSG Inter-core Communication API Return Values
...
- Success: 0
- Failure: negative error code; see RPMSG Inter-core Communication API Return Values
**Description**
...
- Failure: negative error code; see RPMSG Inter-core Communication API Return Values**Description**
...
#### IPCFHAL Inter-core Communication API Return Values
...
- Success: 0
- Failure: <0; see IPCFHAL Inter-core Communication API Return Values
**Description**
...
#### HEAP Allocation API Return Values
...
- Success: 0
- Failure: <0; see HEAP Allocation API Return Values
**Description**
...
#### VDSP Boot/Stop Control API Return Values
...
- Success: 0
- Failure: negative error code; see VDSP Boot/Stop Control API Return Values
**Description**
...
::: tip This API does not support concurrent use from multiple processes or threads. :::
