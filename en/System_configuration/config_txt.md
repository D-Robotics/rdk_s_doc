...
URL: https://developer.d-robotics.cc/rdk_s_doc/en/System_configuration/config_txt
...
3. `boot_config_f` : changes the default configuration filename. For example, `setenv boot_config_f test.txt` will cause the next boot to look for a file named `test.txt` instead of `config.txt` ;
4. `boot_config_dev_part` : changes the default partition where the configuration file is searched. For example, `setenv boot_config_dev_part 0:0xd` will cause the next boot to look for the configuration file in partition 13 (0xd) of the current boot medium;
5. `boot_config_intf` : changes the default storage interface. For example, `setenv boot_config_intf scsi` ;
6. Save the environment variables: `saveenv`
