...
URL: https://developer.d-robotics.cc/rdk_s_doc/en/Basic_Application/multi_media/multi_media_api/pydev/object_camera
...
【Notes】The parameters `width` and `height` support input of type `list` , which enables the camera to output multiple resolutions simultaneously. The `list` supports up to 6 scaled-down outputs, with scaling ratios in the range [1, 1/64) relative to the camera's native resolution. Example usage:
...
【Notes】This API must be called after `open_vps` .
