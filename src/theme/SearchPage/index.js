...
function SearchPageContent() {
  ...
  const {product, version: rdkVersion} = useDocScopeFilter();
  const {
  ...
  const makeSearch = useEvent((page = 0) => {
    ...
    if (product) {
      algoliaHelper.addFacetRefinement('product', product);
    }
    if (rdkVersion) {
      algoliaHelper.addFacetRefinement('rdk_version', rdkVersion);
    }
  ...
  }, [searchQuery, docsSearchVersionsHelpers.searchVersions, makeSearch, product, rdkVersion]);
  useEffect(() => {
