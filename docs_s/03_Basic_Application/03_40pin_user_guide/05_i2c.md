...
- RDX S100 在 40PIN 上默认使能 I2C5（物理管脚号 3 和 5）和 I2C4（物理管脚号 27 和 28），IO电压3.3V。

:::info

在`RDK S100`平台上，存在两种硬件形式， 分别支持30pin和40pin， 其中`40pin`上使用I2C5有以下限制:

- 40pin上需要波动拨码开关来选择使用I2C5还是UART2, 具体细节可以查看下图：
