...
1. 将build_params目录下的`ubuntu-22.04_desktop_rdk-s100_beta.conf`和`ubuntu-22.04_desktop_rdk-s100_release.conf`中的中的PARTITION_FILE配置为OTA版本（PARTITION_FILE="s100-ota-gpt.json）；

    ![ota_conf](../../../static/img/07_Advanced_development/02_linux_development/image/ota/ota_conf.png)

2. 将source/bootloader/device/rdk/s100目录下的`board_s100_debug.mk`和`board_s100_release.mk`文件中的`RDK_OTA`变量配置为开启（export RDK_OTA="yes"）

    ![ota_board_conf](../../../static/img/07_Advanced_development/02_linux_development/image/ota/ota_board_conf.png)
3. 编译
   - 若未编译过工程，没有生成out目录则执行下列命令，编译工程。（若已编译并生成out目录，此步骤可跳过）
        ```bash
        sudo ./pack_image.sh
        ```
   - 制作新的miniboot的deb包
