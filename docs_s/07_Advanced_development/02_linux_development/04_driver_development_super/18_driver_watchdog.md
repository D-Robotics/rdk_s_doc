...
## Watchdog 概述
  ...
  SOC包含13个看门狗，分布如下：Acore共6个、MCU共3个、VDSP共1个、BPU共2个、HSM共1个，Acore Watchdog具体使用情况如下表所示，其中Watchdog0、Watchdog1、Watchdog2默认配置重启，Watchdog3、Watchdog4、Watchdog5未配置重启，需要客户自行配置：
...
在设备树中增加看门狗的硬件信息描述（位置：code/source/kernel/arch/arm64/boot/dts/hobot/drobot-s100.dtsi）
```dts
...
当监控Acore的Watchdog Timeout时，会触发中断发送到MCU0侧，由muc的任务触发acore重启，在任务中会等待5s，使acore侧完成堆栈的打印。
