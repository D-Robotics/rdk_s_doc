...
S600芯片中3个sys包含可软件控制的pin脚，根据用户的需要，可以对pin脚进行功能复用，属性设置和电压域设置。
...
S100/S600中共存在两种电压域，分别为1.8v和3.3v，不同的pin脚需要工作在合适的电压域中。手册中将pin脚划分入power group中，在手册中可以看到每个power group控制着一组pin脚。
...
pinctrl-hobot.c文件中包含了S100/S600共用的pinctrl主逻辑代码，向pinctrl子系统注册设备，并向下提供SoC数据的初始化入口。
pinctrl-hobot-s100.c文件中包含S100的pin、pin group、function定义和设备匹配信息。
...
pinctrl-hobot.c文件中包含S100/S600共用的pinctrl主逻辑代码。
pinctrl-hobot-s600.c文件中包含S600的pin、pin group、function定义和设备匹配信息（`compatible = "drobot,s600-pinctrl"`）。
...
S600 acore中pinctrl设备树文件路径为：`hobot-drivers/kernel-dts/drobot-s600-pinctrl.dtsi`。

S600同样通过`pinmux`和`pinconf`子节点完成配置，但节点命名和功能命名与S100不同，通常按`hsi`/`cam`/`peri`域组织。
...
驱动在使用pinctrl接口前，需要在DTS里配置相应的pinctrl节点。驱动probe时会将`default`状态配置到寄存器，其他状态可在代码中显式切换。
...
驱动在使用pinctrl接口前，需要在DTS里配置相应的pinctrl节点。驱动probe时会将default状态配置到寄存器，其他状态可在代码中显式切换。
...
S100与S600在pinctrl debugfs查询目录结构上保持一致，可直接在目标设备端通过`/sys/kernel/debug/pinctrl/`下相同节点进行查询。实际设备名（如`<pinctrl_dev>`）和输出内容以设备端结果为准。
