...
目前RDK S100/S600有6/10个Instance，每个Instance有4个32bit计数的Channel，每个Channel都是一个定时器，可用于定时等功能。
...
sample 程序默认使用定时器2的通道2（Timer2 Channel2），S100/S600上的定时器输入时钟默认配置为 200MHz/40MHz。
...
- load: 定时器装载值（计数初值），仅在 on 操作时有效
    在S100上默认的时钟频率为200MHz，S600上为40MHz，时钟频率和装载值的关系如下：
    ```c
