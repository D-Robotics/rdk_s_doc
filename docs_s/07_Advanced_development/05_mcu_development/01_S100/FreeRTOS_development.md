...
### 系统中断说明
MCU和Acore/HSM通信依赖IPC，IPC系统服务涉及到的中断可以参考：IPC的相关介绍。这些中断优先级建议配置成比平常的功能类中断优先级高，这些中断本身可以配置成同样的优先级。
...
FreeRtos_OsTask_SysCore_BSW_xms和FreeRtos_OsTask_SysCore_ASW_xms为周期性任务，会根据xms的不同产生周期性的调度。同时周期性任务内部会有工作处理，细节见"MCU FreeRtos系统简介"章节。
...
### 目前已经被占用的区域：
```c
MCU1_VERSION:  org = 0x0C800000, len = 0x60
MCU_ALIVE:     org = 0x0C800000, len = 0x10
     ---MCU0_ALIVE：org = 0x0C800000, len = 0x04；
     ---MCU1_ALIVE：org = 0x0C800004, len = 0x04；
     ---REVERSED：  org = 0x0C800008, len = 0x08；
```
### 使用注意事项
