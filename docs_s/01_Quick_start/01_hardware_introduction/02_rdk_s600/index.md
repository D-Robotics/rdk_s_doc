...
- <font color="red">**V0P1 版本硬件请使用地瓜提供的电源适配器。**</font>
...
- 不支持休眠唤醒功能。
- 在 V0P1 版本硬件中，I2C7 默认已用于 M.2 KEY M 接口、RTC IC 及 风扇 (FAN) 转速控制 IC 的通讯，RTC IC 和 风扇转速控制 IC 的 I2C 地址分别为 0X32 和 0X2F（不含读写位）。
...
5. 在 V0P1 硬件版本中，需使用地瓜提供的 MCU 扩展板。
6. 在 V0P1 硬件版本中，SPI4 无法使用，SPI6 在 MCU 子卡上无法使用。
   :::
...
RDK S600 开发者套件包括一个 MAIN 域的 CAN 总线接口，该接口采用 10Ppin 自锁连接器，具有 4 路 MCU 域的 CAN 接口，其中 120Ω 电阻可以通过 SW7 开关控制是否接入，MAIN-CAN 接口信号定义见[接口说明](#interface)。
...
<font color="red">**V0P1 硬件版本暂不支持 10G base-T。**</font>
