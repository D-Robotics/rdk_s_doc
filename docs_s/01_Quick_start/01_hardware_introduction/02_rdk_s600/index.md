...
RDK S600开发者套件支持12V-28V 电源供电，支持最大电流16A， 4-pin Microfit Connector。

- <font color="red">**V0P1 版本硬件请使用地瓜提供的电源适配器。**</font>
- 地瓜提供电源适配器额定输出电压 24V，输出最高电流8A。
...
默认用于连接支持 PCIe 的 Wi-Fi 和 USB 2.0 的 BT 模组，同时支持 UART 4线 BT 模组，PCIe 支持 Gen3x1。并提供 SDIO 接口、PCM 音频接口，无 LED 和 ALERT 信号。
...
- 2 路 D-PHY 或可配置成1路 D-PHY+1 路 C-PHY。
- 2 路 IIC 接口。
- 4 路 PWM 信号用于相机触发。
- 2 路 sensor error。
- 2 路解串器 lock 信号。
...
2. 当系统处于 deep sleep 时，VDD_MCU_PERI_5V、VDD_MCU_PERI_3V3 供电关闭。
3. VDDIO_MCU_3V3，VDDIO_MCU_1V8 最大供电电流均为 50mA，仅可用于电平转换、上下拉电阻、ADC 分压等小功耗应用；当系统处于 light sleep 模式时，VDDIO_MCU_3V3 和 VDDIO_MCU_1V8 电源保持供电，当系统处于 deep sleep 模式时，VDDIO_MCU_3V3 和 VDDIO_MCU_1V8 电源关闭。
4. 在 V0P1 硬件版本中，需使用地瓜提供的MCU扩展板。
5. 在 V0P1 硬件版本中，SPI4 无法使用，SPI6 在 MCU 子卡上无法使用。 
:::
