...
RDK S600 采用 UFS 作为系统启动介质。

### **显示**

RDKS600 开发板支持 HDMI 显示接口。通过对应的线缆将开发板与显示器相连接，可实现图形化桌面显示。

### **网络连接**

RDK S600 开发板支持以太网、Wi-Fi 两种网络接口，用户可通过任意接口实现网络连接功能。
...
RDK S600 套件目前提供 Ubuntu 22.04 系统镜像，可支持 Desktop 桌面图形化交互。
...
RDK S600 开发套件可借助 PC 端工具 D-Navigation 或 Xburn 来完成 Ubuntu 系统的烧录工作。当前，该烧录过程支持两种 USB 下载模式，用户可在烧录工具的 “下载选取” 界面里的 “下载模式” 选项处进行选择。这两种模式的具体区别如下：

- **U-Boot 烧录方式：** 此模式依赖 RDK S600 进入 U-Boot 的烧录模式（即 fastboot 模式），在日常的烧录场景中使用较为频繁，能满足大多数常规的系统烧录需求。
- **USB 烧录方式：** 该模式基于 DFU 协议，当 RDK S600 遇到无法进入 U-Boot 模式，或者系统损坏导致设备变砖等特殊情况时，使用此模式帮助恢复系统。


下面给出使用 PC 工具 D-Navigation 和 Xburn 烧录的具体烧录步骤。


#### 烧录准备

在烧录 S600 系统镜像之前，需要做如下准备
...
下载镜像烧录工具 D-Navigation 和 Xburn （可[点击此处下载](https://archive.d-robotics.cc/downloads/software_tools/download_tools/)）

D-Navigation 和 Xburn 均支持 Window，Ubuntu 和 MacOS，根据系统不同，
...
Xburn 已提供 Windows，Ubuntu，MacOS 平台安装包，用户根据自己使用平台选择下载，以 Xburn-1.1.8 为例
- xburn-gui_1.1.8_x64-setup.exe
- xburn-gui_1.1.8_amd64.deb
- xburn-gui_1.1.8_universal.dmg

:::info 注意

各个平台所需要依赖的驱动与工具，请参考[驱动下载与安装](#驱动下载与安装)
...
1. 从网页上[下载](#镜像下载)镜像包
      ![](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/07_Advanced_development/05_mcu_development/01_S600/basic_information/download_web.png)
...
1. 打开 D-Navigation，设置方法如下：
   ...
   ![image-S100-download](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/install_os/image-S600-download.png)
...
- 需要短接板载跳线帽，使 RDK S600 开机进入 DFU 启动模式

- 适用于空片烧录或者固件损坏无法进入 Uboot 情况

- 支持使用 D-Navigation 和 Xburn
...
![image-S600-dfu-mode](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/install_os/image-S600-dfu-mode.png)



**方法1 使用 D-Navigation 进行 USB 烧录**

打开 D-Navigation，设置方法如下：
   ...
   ![image-S600-download_dfu](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/install_os/image-S600-download_dfu.png)


**方法2 使用 Xburn 进行 USB 烧录**
...
打开 Xburn，设置方法如下：
   ...
   ![image-S600-xburn-download_dfu](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/install_os/image-S600-xburn-download_dfu.png)
...
**product 目录路径**
- 预编译 firmware 位于 `SDK目录\board_support_package\firmwares`
- SDK 编译 firmware 位于 `SDK目录\platform_source_code\out\product`

用户根据自己需要选择对应 firmware

:::

   - 点击开始升级，设备上电并等待升级完成
   ![image-S600-xburn-progress](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/install_os/image-S600-xburn-burn_progress.png)



   - 升级完成后，关闭电源，拔除板载跳线帽(从 DFU 模式退出)，重新上电。



### Miniboot 及文件系统升级

D-Navigation 工具支持对 S600 进行[Miniboot 镜像](/rdk_s/Advanced_development/rdk_gen#765-自定义分区说明)更新，在客户需要保留根文件系统修改（例如自行安装的 python/deb 包）时，可以在板端使用`sudo apt update && sudo apt upgrade`进行文件系统升级后，使用 D-Navigation 工具进行 Miniboot 镜像升级。
...
- Uboot 烧录示例如下图：
  ![](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/install_os/image-s600-download_uboot_miniboot.png)

- USB 烧录示例配置如下图：
  ![](https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/install_os/image-s600-download_dfu_miniboot.png)
