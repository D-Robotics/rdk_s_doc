...
编辑 `~/.bashrc` 或 `/etc/environment` 文件，两者的写法与生效方式不同：

- 如果是为当前用户配置代理，编辑 `~/.bashrc`，变量须加 `export` 前缀，否则子进程（如 `apt`、`curl`）读不到代理设置：
...
- 如果是为所有用户配置代理，编辑 `/etc/environment`，变量直接以 `KEY=VALUE` 形式书写（不加 `export`）。该文件由 PAM 在登录时读取，重新登录后生效，执行 `source ~/.bashrc` 不会使其生效：
