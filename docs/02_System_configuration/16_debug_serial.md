...
**适用读者**：需要通过串口登录板卡，或在系统无法启动时进行恢复的开发者。

**前置条件**：一台 PC 与支持数据传输的 USB Type-C 数据线；PC 端已安装 CH340 驱动与串口工具。

**与其他模块关系**：登录账户见[用户与权限管理](./14_user_permission.md)；启动日志的查看见[系统日志查看](./15_system_log.md)。
