...
**现象**：程序启动后输出 `Error: init IMU 'bmi08x' failed !!! Quit Now` 并退出。

**原因**：未接入 IMU 传感器，或对应 IIO 驱动未加载。

**解决**：确认 IMU 传感器已连接，并检查 `/sys/bus/iio/devices/` 下是否存在 `iio:device*` 节点。

### 如何读取非默认型号的传感器

**现象**：需要读取 ICM42688 等其它型号传感器，但不确定如何指定。

**原因**：程序默认使用 `bmi08x`，其它型号需通过 `-n` 参数指定。

**解决**：通过 `-n` 指定传感器名，支持 `bmi08x`、`icm42688-gyro`、`icm42688-accel`。
