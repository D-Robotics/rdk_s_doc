...
| -c                | Channel：声道数，分为单声道 mono 和立体声 stereo，还有多声道如 8/16 通道数 |                                         |
...
默认声卡/设备号是0,0，如果实际场景接入多个声卡设备，需要通过 -D 指定设备、声卡号。
:::
...
S100 适配微雪电子生产的 Audio Driver HAT REV2 音频转接板，通过 40PIN 与 S100 开发板连接。子板介绍和具体连接方法参考：[音频子板使用说明](../../07_Advanced_development/04_driver_development/09_driver_audio.md#音频子板使用说明)

### 设备节点

该音频板接入 S100 加载驱动后，生成的设备节点：
...
表示录制 5s，48k/2ch/16bit 位宽的 wav 音频文件。数据格式用户可以根据自己的需求调整并设置。
...
- 声卡识别成功标志：`arecord -l` 能列出 `card 0: s100snd2`（示例输出见上文「控制命令」）。
- 录音/播放成功标志：录音命令执行后在当前目录生成 `test.wav` 文件，`aplay -Dhw:0,1 test.wav` 播放无报错即为输出正常。

</DocScope>

<DocScope products="RDK S600">

- USB 声卡加载成功标志：`ls /dev/snd` 出现 `pcmC*D*` 节点。
- 录音/播放成功标志：`arecord -Dhw:1,0 --dump-hw-params` 能列出硬件支持的通道数/采样率/位宽，录音命令执行后生成 `test.wav` 文件，播放命令正常执行无报错。
...
**原因**：音频设备未连接或连接不良，Audio Driver HAT REV2 场景下拨码开关配置错误。

**解决**：检查硬件连接与拨码开关设置，确认 `arecord -l` 能列出对应声卡；S600 USB 声卡场景确认 `ls /dev/snd` 存在 `pcmC*D*` 节点。

### 录音或播放无声

**原因**：音频文件格式、通道数与命令参数不一致。

**解决**：确认命令中的 `-c`（声道数）、`-r`（采样率）、`-f`（位宽）与音频文件实际属性一致，并用 `arecord --dump-hw-params` 核对硬件支持范围。

<DocScope products="RDK S100">

### 回采通道无信号

**原因**：未使用 8 通道录音命令，或同一组 I2S 下位深、采样率、通道数未对齐。

**解决**：改用 8 通道录音命令（回采信号映射在录音通道 7、8），并保持通道数、位深、采样率对齐。
