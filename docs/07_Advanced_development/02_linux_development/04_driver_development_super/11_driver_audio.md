...
可查看当前 substream 的状态(running、xrun 等)，appl_ptr/hw_ptr 指针的值。其中，hw_ptr 和 app_ptr 可以识别底层硬件无法传输或接收任何数据的情况。
比如，启动测试但是中断未正常触发，数据传输将会停滞。此时，hw_ptr/appl_ptr 指针将持续得不到更新
...
以录音为例，写文件创建单独的线程和创建 ring buffer(ring buffer 大小可自由调整)，pcm_read 的 buffer 写入 ring buffer，fwrite 从 ring buffer 读取数据。只要 fwrite 陷入时间不超过 ring buffer 的限制，就不会发生 xrun 导致数据。
