...
:::info
本文示例以 RDK S100 为例；`lsusb` / `lsblk` / `dd` / `iperf3` 等命令在 RDK S600 上同样适用，仅示例中的板端 USB 拓扑与 IP 地址不同，请以实际板端为准。较新系统镜像中 `ifconfig` 已废弃，建议使用 `ip addr` 替代。
:::
...
**原因**：受 U 盘本身性能、接口速率、测试块大小与文件大小等因素影响。

**解决**：参考「常见影响因素」逐项排查，使用较大的块大小与文件进行读写测试。

### U 盘接入后未自动挂载

**原因**：未自动挂载或设备未识别。

**解决**：`lsblk` 查看设备，手动 `mount` 到挂载点后再测试。
