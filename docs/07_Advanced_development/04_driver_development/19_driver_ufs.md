...
S100 芯片内置 UFS Host 控制器，硬件最高支持 UFS 3.1 协议，软件接口最高支持 3.0，支持 HS-G4 速率模式与 2 路数据通道。
</DocScope>

<DocScope products="RDK S600">
S600 芯片内置 UFS Host 控制器，硬件最高支持 UFS 3.1 协议，软件接口最高支持 3.0，支持 HS-G4 速率模式与 2 路数据通道。
</DocScope>

**适用读者**：模式 3 深度定制开发者（商业客户/深度团队）——需要调试 UFS 驱动、固件适配、性能或可靠性问题的 BSP/驱动工程师。

**前置条件**：已烧录 RDK OS 并可登录板端；了解 Linux UFS/SCSI 子系统与设备树基础。

**与其他模块关系**：本驱动是板端存储（`/dev/sda`）分区挂载的底层实现，引导/根文件系统依赖它；内核配置入口见「[配置 U-Boot 和 Kernel](./01_uboot_kernel_config.md)」。本文档介绍 UFS 驱动的开发、配置和调试方法。
...
| `--size` | 测试文件大小 | 512M, 1G |
| `--numjobs` | 并发任务数 | 1, 4, 8 |
| `--ioengine` | IO 引擎 | libaio, sync |
| `--direct=1` | 直接 IO（绕过缓存） | 1 |
| `--runtime` | 测试运行时间(秒) | 60 |
...
2. **调整 PHY 参数**
   ...
   - 在 DTS 中配置合适的`tx_eq_main_pre_post`参数
...
**原因**：UFS 设备未正确连接（UFS 不支持热插拔），或驱动未正确初始化。

**解决**：`lsblk` 查看块设备，`dmesg | grep -i ufs` 查看内核日志确认 `ufshcd` 相关输出；确认启动前设备已正确接入。
