...
需要注意的是 VDSP FW 与其他模块共用一个串口，如 BL31、optee，若 VDSP FW 输出日志太多，可能会阻塞这些模块的日志输出，引起 watchdog。
另外 VDSP FW 日志和 kernel 日志都输出到串口中，存在相互干扰的问题，用户可通过降低 kernel 日志等级来防止日志干扰：
...
:::tip
调试使用 xos 工具时，提示找不到工具链，先确认 xos build 环境是否建立。
...
#### 核间通信 RPMSG API 返回值 {#rpmsg_api_return_value}
...
#### 核间通信 IPCFHAL API 返回值 {#vdsp_ipcfhal_api_return}
...
#### HEAP 分配 API 返回值 {#vdsp_heap_api_return}
...
#### VDSP 启停控制 API 返回值 {#vdsp_boot_api_return}
