...
如果你需要在开发板上编译内核模块或类似的代码，你需要安装 Linux 内核头文件。这些头文件包含 Linux 内核的各种常量定义、宏定义、函数接口定义和数据结构定义，是完成内核模块代码编译所必需的依赖代码。
...
:::note
模块签名仅旧版本需要。RDK S600 从 5.1.1 起、RDK S100 从 4.1.0 起已移除模块签名校验，驱动默认不加密。新版本编译出的 `.ko` 文件无需签名即可直接加载。以下签名步骤仅适用于旧版本。
:::
...
2. 根据 `peak-system` 官方说明，下载 `pcan-kernel-version.sh` 脚本，并确认需要下载的驱动版本：
    ...
    ```
    :::info
    地瓜 RDK S100/S600 系列仅支持 Linux 6.1 版本内核。
    :::
3. 下载 PCAN 驱动
    ```shell
...
4. 解压 PCAN 驱动压缩包，这里我们以 2025-08-16 的 PCAN 驱动版本“8.20.0”为例，请用户根据下载的实际文件修改命令。
    ```shell
...
5. 执行编译：
    ...
    sudo make install
    ```
6. 对 PCAN 驱动进行签名并使用，以下示例以 8.20.0 版本的 PCAN 驱动为例，该版本下，PCAN 驱动模块被默认安装到了路径 `/lib/modules/6.1.158-rt58/misc/` 内。签名仅旧版本需要，新版本驱动默认不加密，直接执行第 3 步即可：
...
**原因**：驱动模块未签名，而板端开启了模块签名校验（仅旧版本，详见 [模块签名](#模块签名) 的版本说明）。
