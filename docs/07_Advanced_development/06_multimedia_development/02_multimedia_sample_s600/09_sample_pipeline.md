...
示例：以连接到 mipi host 2，link port 1 的 ovx3cstd Serdes sensor 为输入，则执行`./single_pipe_vin_isp_ynr_pym_vpu -s 4 -m 2 -l 1`。
...
示例 : 以连接到 mipi host 2，link port 1 的 ovx3cstd Serdes sensor 为输入，执行 `./single_pipe_vin_isp_ynr_pym_gdc -s 4 -m 2 -l 1 -f ../../vp_sensors/gdc_bin/ovx3cstd.bin`
...
示例 : 以 连接到 mipi host 2，link port 1 的 ovx3cstd Serdes sensor 为输入，执行 `./single_pipe_vin_isp_ynr_pym_gdc_vpu -s 4 -m 2 -l 1 -f ../../vp_sensors/gdc_bin/ovx3cstd.bin`
...
示例 :
1. 以 imx219 （ MIPI 类型） 和 连接到 mipi host 2，link port 1 的 ovx3cstd (Serdes 类型 ) 作为 sensor 输入
2. 执行命令 `./multi_pipe_vin_isp_ynr_pym_gdc_vpu -c "sensor=0" -c "sensor=4 mipi=2 link=1"`
...
将 UVC camera 接入开发板的 USB 接口（J7/J8/J9）即可使用本示例，接口详情参考 [USB 3.0 Type-A 接口 (J7/J8/J9)](/01_Quick_start/01_hardware_introduction/02_rdk_s600/01_rdk_s600_kit#usb-30-type-a-接口-j7j8j9)。
