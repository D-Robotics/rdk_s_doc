...
以 imx219 sensor 为例，则执行 `./get_vin_data -s 0` 。

以 连接到 mipi host 2，link port 1 的 ovx3cstd Serdes sensor 为例，则执行`./get_vin_data -s 4 -m 2 -l 1`。
...
Options:
-c, --config="sensor=id link=port mipi=mipi_rx"
