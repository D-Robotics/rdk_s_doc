...
- **context**：`media_codec_context_t` 编解码上下文，贯穿整个生命周期。
- **状态机**：INITIALIZED → CONFIGURED → 运行（start/pause/flush/stop）→ release。
- **VPF 接入**：`hb_mm_mc_vpf_init` 将 MediaCodec 与 VPF 通道关联。
- **编解码器注册**：`hb_mm_mc_register_audio_encoder/decoder`/`unregister_*` 动态注册/去注册音视频编解码器。

## API 调用流程

1. `hb_mm_mc_initialize` 初始化 context（进入 INITIALIZED）。
2. `hb_mm_mc_vpf_init` 关联 VPF 通道（如需）；`hb_mm_mc_configure` 配置（进入 CONFIGURED）。
3. `hb_mm_mc_start` 启动；运行中 `hb_mm_mc_pause`/`hb_mm_mc_flush`/`hb_mm_mc_request_idr_frame`。
4. `hb_mm_mc_stop` 停止；`hb_mm_mc_release` 释放 context。

## 快速示例

以下示例参考板端 `/app/multimedia_samples/sample_codec/sample_codec.c`，演示编码器（encoder）的最小使用序列：
...
> 解码器将 `context.encoder` 置为 `false`、`codec_id` 换为解码 ID 即可；buffer 类型编码为 `MC_VIDEO_FRAME_BUFFER`、解码为 `MC_VIDEO_STREAM_BUFFER`。完整流程见 `sample_codec.c`。
