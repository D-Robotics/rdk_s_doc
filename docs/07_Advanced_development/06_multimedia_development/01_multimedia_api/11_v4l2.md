...
sidebar_position: 11
title: "V4L2"
description: "RDK S100/S600 基于标准 V4L2 框架的视频采集：驱动切换、场景配置、buffer 分配与取流示例"
---
...
```bash
rmmod vid_v4l2
modprobe vid_v4l2 scene_table="<场景表字符串>"
```
...
以下为标准 V4L2 mmap 方式取流的最小调用序列（板端参考 `sample_pipeline/uvc_capture_sample/v4l2_common_utils.c`）：
...
// 2. 申请 buffer 并 mmap（ion/dma 两种方式见「v4l2 buffer 分配方式」）
...
for (int i = 0; i < 4; i++) {
    ...
    planes[i] = mmap(NULL, buf.length, PROT_READ | PROT_WRITE,
...
for (int i = 0; i < 120; i++) {
    ...
    ioctl(fd, VIDIOC_DQBUF, &dq);       /* 阻塞取帧 */
    /* 处理 planes[dq.index] ... */
    ioctl(fd, VIDIOC_QBUF, &dq);        /* 归还 buffer */
}
