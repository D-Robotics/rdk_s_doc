...
sidebar_position: 1
title: "硬件、系统与环境配置"
description: "RDK 板卡硬件接口、系统启动与烧录、系统配置、外设与编译环境的常见问题与排查方法"
---

# 硬件、系统与环境配置

本节解答 RDK 板卡在硬件、系统与环境方面的常见问题，按产品与系统版本、硬件使用与接口、系统启动与烧录、系统配置与环境、外设、存储与显示、编译与开发环境、其他通用问题分组。

:::info 说明

🔄 问题解决前请考虑更新到最新系统：许多问题可通过系统更新解决，相关下载资源请参考[下载资源汇总](../RDK.md#资料索引)。
...
认证配件及购买链接请参考 [RDK S100 认证配件清单](https://horizonrobotics.feishu.cn/wiki/V4PZw5vBEiUV4akdvgOcr77zncf)。

</DocScope>

<DocScope products="RDK S600">

认证配件及购买链接请参考 [RDK S600 认证配件清单](https://horizonrobotics.feishu.cn/wiki/V4PZw5vBEiUV4akdvgOcr77zncf)。

</DocScope>

## 产品与系统版本

### 什么是 D-Robotics RDK 套件

D-Robotics Developer Kits，简称 [D-Robotics RDK 套件](../RDK.md)，是基于 D-Robotics 计算平台打造的机器人开发者套件。

### 如何查看 RDK 板卡的系统版本号

登录 RDK 板卡的系统后，可使用以下命令：
...
1. **查看系统大版本号**：
    ...
    RDK OS 4.x/5.x 输出类似 `4.0.5`、`5.1.0`。
...
2. **查看已安装的地瓜核心功能包版本**：
    ...
    [Hardware Model]:
            D-Robotics RDK S100 V0P6 (Board Id = 0x6A85)
    ...
    ...
    [RDK Kernel Version]:
            Linux ubuntu 6.1.158-rt58-DR-4.0.6-2603301938-ga9839d-gfb172a #20 SMP PREEMPT_RT Mon Mar 30 19:40:19 CST 2026 aarch64 aarch64 aarch64 GNU/Linux

    [RDK Miniboot Version]:
            4.0.6-20260331113043

    [RDK Packages List]:
            hobot-audio-config/now 4.0.1-20250514184314 arm64 [installed,local]
            hobot-camera/now 4.0.6-20260330194630 arm64 [installed,local]
            hobot-configs/now 4.0.5-20260922205131 arm64 [installed,local]
            hobot-dnn/now 4.0.5-20260922175655 arm64 [installed,local]
...
RDK OS 4.x/5.x 为现行主线版本：RDK OS 4.x 面向 RDK S100，RDK OS 5.x 面向 RDK S600。

**版本升级**：推荐使用烧录新版本系统镜像的方式进行升级，参见[系统烧录](../01_Quick_start/03_install_os_and_setup/02_burn.md)。

**TROS 兼容性**：各 TROS 版本与 RDK OS 大版本的对应关系，参见 TROS 手册的[版本发布记录](https://developer.d-robotics.cc/tros_doc/quick_start/changelog?v=4.0.5&p=RDK+S100)。

## 硬件使用与接口

### 摄像头插拔注意事项

**原因**：**严禁在开发板未断电的情况下插拔摄像头，否则非常容易烧坏摄像头模组或主板接口。**

**解决**：请务必在断开开发板所有电源后，再进行摄像头的连接或移除操作。

### 板卡高负载运行时温度过高

**原因**：板卡温度过高会影响性能稳定性，甚至可能损坏硬件。
...
3. **监控温度**：
    * 使用系统命令（如 `hrut_somstatus`）来实时监控芯片温度。
...
“发热量小不代表温度会低”。即使芯片本身设计功耗不高，如果散热不良，热量积聚仍然会导致表面温度快速升高。良好的散热设计是保证嵌入式系统稳定运行的关键。

### IMX219 等 MIPI 摄像头的连接与验证

**解决**：IMX219 这类 MIPI 摄像头模组通常通过 24pin FPC（柔性扁平电缆）与开发板连接。

**连接注意**：FPC 排线的两端通常有蓝色或黑色加强筋，请确保**加强筋的一面朝上**（或朝向连接器卡扣的扳手面，具体取决于连接器类型）插入到开发板和摄像头模组的连接器中，并锁紧卡扣。

IMX219 摄像头连接示意图：

<img src="http://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/08_FAQ/image/hardware_and_system/mipi_connect.png" alt="IMX219 camera connected to RDK S100" style={{ width: '80%', maxWidth: '980px', height: 'auto', display: 'block', margin: '0 auto' }} />
...
3. **通过 `i2cdetect` 命令检查 I2C 通信**：
    ...
    MIPI 摄像头通常通过 I2C 总线与主控芯片通信以进行配置。您可以使用 `i2cdetect` 命令来扫描连接到特定 I2C 总线上的设备。RDK S100 上 MIPI 摄像头常用的 I2C 总线可能是 `i2c-1` 或 `i2c-2`（具体请查阅硬件接口定义）。
...
**原因**：这类问题通常与供电、启动介质（SD 卡/eMMC）或硬件连接有关。
...
### 系统启动卡在 `hobot>` U-Boot 命令行

**原因**：当系统启动时停留在 `hobot>` 提示符，这表示板卡已进入 U-Boot（Universal Boot Loader） 的命令行模式，而没有继续引导加载 Linux 内核。可能的原因有：
...
**原因**：地瓜机器人官方软件源域名或 GPG 签名密钥发生变更，导致本地配置过期。
...
**原因**：ROS2 官方软件源 GPG 签名密钥更新，导致本地配置过期。
...
为应用配置开机自启动的方法，见[开机自启动配置](../02_System_configuration/06_self_start.md)。
...
**原因**：RDK 的 Ubuntu 系统中通常默认已经编译并可能启动了 `adbd` （ADB 守护进程） 服务，但其默认配置和 USB 接口的功能模式可能需要调整才能用于 ADB 连接。
...
**注意**：具体的启用步骤可能因 RDK 型号和系统版本而异。请务必查阅对应板卡和系统版本的官方文档中关于 ADB 功能配置的详细说明。有时，官方提供的 `bootloader` 更新教程中也可能包含 ADB 的配置或使用前提。参考（来自原始文档，可能主要关于 bootloader 更新，但可间接涉及 ADB 环境）：[bootloader 镜像更新](https://developer.d-robotics.cc/forumDetail/88859074455714818)（建议查找更专注于 ADB 配置的官方文档或社区帖子）。

### 执行 `apt upgrade` 时桌面环境黑屏

**原因**：直接在图形桌面的终端中更新，有时在升级桌面自身相关的包时可能会导致显示中断或 X Server 重启，从而造成黑屏现象。

**解决**：建议通过串口终端或 SSH 远程登录后，在纯字符界面的终端中执行系统更新命令（如 `sudo apt update && sudo apt upgrade`）。

### Server 版能否直接升级为 Desktop 版

RDK OS 的 Server 版和 Desktop 版在预装的软件包上存在显著差异，最主要的是 Desktop 版包含了图形用户界面（如 XFCE 桌面环境）及其相关组件，而 Server 版通常不包含这些，以节省资源。
...
**原因**：Docker 需要使用 iptables 的 legacy 模式。

**解决**：用户可以使用以下命令修复 Docker 运行：
...
**解决**：如果需要手动配置，请注释掉 `DefaultEnvironment="TZ=CST-08:00"`，然后 `reboot` 重启设备使配置生效。

## 外设、存储与显示

### 挂载 NTFS 文件系统的 U 盘或硬盘并支持读写

**原因**：Ubuntu 系统默认对 NTFS 文件系统的写入支持可能不完整或仅为只读。

**解决**：为了获得完整的 NTFS 读写支持，需要安装 `ntfs-3g` 软件包。
...
### 连接 HDMI 显示器后无画面或显示异常

**原因**：HDMI 显示问题可能由显示器兼容性、线缆、系统配置、供电或硬件故障造成。
...
3. **RDK 系统配置**：
    ...
    * 在 RDK OS 2.1.0 及以后版本（含现行 4.x/5.x），如果遇到显示不兼容，可以尝试先通过 VNC（Virtual Network Computing）连接到板卡（如果已开启），然后在系统中手动调整 HDMI 的输出分辨率。参考：[HDMI 显示问题及分辨率调整](https://developer.d-robotics.cc/forumDetail/204825652464181769)
4. **供电问题**：虽然不直接，但严重的供电不足可能导致系统无法正常初始化显示子系统。
5. **硬件问题**：极少数情况下，可能是板卡的 HDMI 接口或显示器本身的硬件故障。

### HDMI 显示器不被支持时抓取 EDID 信息

**原因**：如果您的 HDMI 显示器无法正常显示或显示异常，技术支持人员可能需要您提供该显示器的 EDID（Extended Display Identification Data） 信息来帮助诊断问题或添加兼容性支持。EDID 包含了显示器的特性、支持的分辨率和时序等信息。
...
获取到 EDID 数据（通常是一个二进制文件或十六进制文本）后，可以将其提供给技术支持。具体在地瓜机器人 RDK 平台抓取 EDID 的方法，请参考官方论坛的指导帖子：[如何提供不支持显示器的 EDID 信息](https://developer.d-robotics.cc/forumDetail/235046352323895808)

### RDK S100 桌面 Power Statistics 应用设备节点显示不全

**原因**：RDK S100 默认不支持，未提供相应的驱动。

**解决**：如用户需要支持显示适配器信息和电池信息，请自行联系电源管理芯片供应商提供驱动。

## 编译与开发环境

### 在开发板上本地安装 VS Code 与远程连接

**解决**：

* **本地安装 VS Code**：RDK 开发板作为嵌入式 ARM 架构设备，通常**不直接支持**在其上本地安装并运行完整版的 VS Code 桌面应用程序。VS Code 官方主要提供 x86_64 架构的桌面版。
* **远程开发（推荐方式）**：强烈推荐在您的 PC（Windows、macOS 或 Linux）上安装 VS Code，并利用其强大的 **Remote - SSH** 扩展插件来远程连接到 RDK 开发板。通过这种方式，您可以在 PC 上享受完整的 VS Code 体验（包括代码编辑、智能提示、调试器前端等），而实际的代码编译、运行和调试则在 RDK 板卡上执行。
...
### 在 Conda 虚拟环境中使用 RDK 特定 Python 包

**原因**：地瓜机器人官方提供的 `Hobot.GPIO`（GPIO 控制）、`hbm_runtime`（智能计算架构推理运行时，历史版本中为 `hobot_dnn`）等 Python 包通常是为 RDK 的系统 Python 环境预编译和优化的，它们可能依赖于系统底层的特定库文件和驱动程序。在 Conda 等 Python 虚拟环境中使用这些包可能会遇到一些挑战，因为虚拟环境旨在隔离依赖。
...
2. **尝试在 Conda 环境中通过 `pip` 安装系统路径下的包（如果 `.whl` 不可用）**：
    * 如果这些包已安装在 RDK 的系统 Python 环境中（例如 `/usr/lib/python3/dist-packages/` 或类似路径），且 Conda 环境的 Python 版本与这些包编译时所用版本兼容，可以尝试在激活 Conda 环境后用 `pip` 指向这些包的路径安装。**但这种方式通常不被推荐**，成功率也不高，因为 `pip` 主要用于从 PyPI 或本地 `.whl`/源码包安装。
...
**原因**：较新版本的 Linux 内核，尤其是在启用了 Secure Boot 的系统上，或者某些发行版的默认安全策略，会要求加载到内核中的模块（`.ko` 文件）必须具有有效的数字签名。如果您自行编译了一个内核模块但没有对其进行签名，在尝试使用 `insmod` 或 `modprobe` 加载时就可能遇到此类错误。
...
* 地瓜机器人官方 RDK 文档中关于“Linux 开发”或“驱动开发”的章节，也可能包含针对 RDK 平台的具体模块签名指导：
    ...
    [内核头文件与模块编译](../07_Advanced_development/03_system_software/09_kernel_headers.md)（请查找此文档中关于模块签名的具体章节）。

    </DocScope>

### 编译大型项目或运行内存密集工具时内存不足

**原因**：内存不足（Out of Memory, OOM）是嵌入式设备或资源受限的开发机上编译大型项目或运行内存密集型应用时常见的问题。
...
**解决**：为 RDK 板卡（通常是 ARM 架构）交叉编译应用程序，一般需要一台 x86 架构的 Linux 开发主机。推荐使用 Ubuntu LTS 版本（如 Ubuntu 20.04 或 22.04），并在其上配置交叉编译工具链和对应的目标系统 SDK（Sysroot）。具体配置步骤因程序类型（如普通 Linux C/C++ 程序、ROS/TROS 功能包）和目标 RDK 的型号与系统版本而异。

1. **编译普通 Linux C/C++ 应用程序**：
    * **获取交叉编译工具链**：地瓜机器人官方会为 RDK 系列平台提供相应的交叉编译工具链（例如，包含 `aarch64-linux-gnu-gcc`、`aarch64-linux-gnu-g++` 等工具）。这个工具链可能作为 SDK 的一部分提供，或者需要从开发者社区单独下载。
...
2. **编译 ROS/TROS 功能包**：
    * **使用官方提供的 Docker 交叉编译环境（强烈推荐）**：这是为 TROS 功能包进行交叉编译**最推荐且最便捷**的方式。地瓜机器人官方通常会提供预配置好的 Docker 镜像，这些镜像中已经集成了：
        ...
        * **参考官方手册**：TROS 用户手册中关于“源码安装”、“开发者指南”或“交叉编译”的章节通常会有详细的 Docker 使用方法和命令示例。参见 [TROS 手册](https://developer.d-robotics.cc/tros_doc/tros)。
...
**解决**：在遇到 RDK 相关问题并寻求帮助前，建议进行以下预排查。

1. **查阅最新官方手册**：确保您参考的是官方最新版本的用户手册、开发文档和发行说明。官方文档通常会包含最新的信息和已知问题的解决方案。您可以从 RDK 资料中心获取最新文档：[RDK 资料中心](https://developer.d-robotics.cc/rdk_doc_center/)
...
3. **仔细检查硬件连接**：确保所有硬件连接都牢固可靠，包括电源线、调试串口线、摄像头排线、网络线以及其他外设连接。接触不良是许多问题的根源。
...
### Docker 镜像、OE 包或 Samples 包下载失败或缓慢
