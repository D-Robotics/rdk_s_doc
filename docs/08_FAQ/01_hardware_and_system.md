...
sidebar_position: 1
title: "硬件、系统与环境配置"
description: "硬件、系统与环境配置 常见问题与排查"
---
...
认证配件及购买链接请参考[RDK S100 认证配件清单](/01_Quick_start/01_hardware_introduction/01_rdk_s100/01_rdk_s100_kit#accessory-list)
...
**A:**
* **本地安装 VS Code**： RDK 开发板作为嵌入式 ARM 架构设备，通常**不直接支持**在其上本地安装并运行完整版的 VS Code 桌面应用程序。VS Code 官方主要提供 x86_64架构的桌面版。
* **远程开发 (推荐方式)**：
...
参考 (来自原始文档，可能主要关于 bootloader 更新，但可间接涉及 ADB 环境)：[bootloader镜像更新](https://developer.d-robotics.cc/forumDetail/88859074455714818) (建议查找更专注于 ADB 配置的官方文档或社区帖子)。
...
具体在地瓜机器人 RDK 平台抓取 EDID 的方法，请参考官方论坛的指导帖子：[如何提供不支持显示器的EDID信息](https://developer.d-robotics.cc/forumDetail/235046352323895808)
...
3.  **SD 卡兼容性**：
    ...
    * 如果遇到 SD 卡兼容性问题，建议优先使用官方推荐镜像和可靠存储介质。
...
### Q24: 编译自己开发的 Linux 内核模块（`.ko`文件）后，加载时遇到“驱动签名报错”或“Required key not available”等问题怎么办？
...
5.  **使用更高配置的开发机/服务器进行编译**：
    如果是在 x86开发机上为 RDK 进行交叉编译，而开发机本身内存也有限，可以考虑使用内存配置更高的机器。对于板端编译，如果资源实在不足，交叉编译是更好的选择。
...
2.  **编译 ROS/TROS 功能包**：
    * **使用官方提供的 Docker 交叉编译环境（强烈推荐）**： 这是为 TROS 功能包进行交叉编译**最推荐且最便捷**的方式。地瓜机器人官方通常会提供预配置好的 Docker 镜像，这些镜像中已经集成了：
        ...
        * **参考官方手册**： TROS 用户手册中关于“源码安装”、“开发者指南”或“交叉编译”的章节通常会有详细的 Docker 使用方法和命令示例。例如，此链接可能包含相关信息：[TROS 手册](https://liqinglian01.github.io/tros_doc/tros)。
...
**A:** IMX219这类 MIPI 摄像头模组通常通过24pin FPC（柔性扁平电缆）与开发板连接。
...
- 供电电压需控制在 12~20V。超过 20V 会导致开发板损坏；低于 12V 可能导致开机异常（如电源上电异常、开机未进入 kernel 等）。
- 不可超出地瓜提供的最大供电电流，否则可能导致开发板损坏或开机异常。
- 对外接口所有 PIN 脚能输出的最大电流即外设能获取的最大电流，无法通过并联多个 PIN 脚提供更大的电流能力。

:::

:::note 接口与传感器接入

- 各接口的具体注意事项，见对应接口介绍部分的「提示」或「注意」信息。
- 接入传感器前，请确认供电是否满足传感器本身的电压、电流、电平幅度等电气特性要求，并检查硬件电路连接是否正确。
- USB 设备异常时，请检查硬件连接是否松动、USB 连接线是否过长、开发板能否满足 USB 设备功耗需求、USB 软件配置是否准确，也可更换不同 USB 设备确认。

:::

:::note Pinlist 说明

- Pinlist Excel 文件中的 Pull up/down 列，表示接口信号上电时的默认上拉或下拉状态。
- 引脚功能缩写含义：`AO` 模拟输出、`AI` 模拟输入、`I` 数字输入、`O` 数字输出、`IO` 数字输入或输出、`NULL` 空值、`/` 无引脚或无状态。

:::

:::tip 扩展板连接

CAM 子板与 MCU 子板与 RDK S100 主板的连接方法，可参考 [RDK S100 相机扩展板](/Quick_start/hardware_introduction/rdk_s100/rdk_camera_expansion_board/rdk_camera_expansion_board) 与 [RDK S100 MCU 接口扩展板](/Quick_start/hardware_introduction/rdk_mcu_port_expansion_board) 章节的参考视频。

:::

:::warning 连接与短路防护

- 扣接 CAM 子板和 MCU 子板时，确认扣接位置准确无松动后再给主板供电；若不需要子板，可直接给主板供 12~20V 电源并开机。
- 使用时注意周边是否有与开发套件电路直接接触的导体，若有请将开发套件移至远离裸露导体的位置，避免短路。
