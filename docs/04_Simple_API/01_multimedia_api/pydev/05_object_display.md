...
**现象**：调用 `set_img` 后无画面或显示异常。

**原因**：使能分辨率与显示器实际分辨率不匹配，或送入数据不是 `NV12` 格式，或显示功能未使能。

**解决**：使能分辨率与显示器 EDID 支持的分辨率匹配（1920x1080、1280x720、1024x600、800x480），数据用 `NV12`，并确保先调用 `display` 使能显示。

### 绘制字符显示乱码

**现象**：调用 `set_graph_word` 绘制字符显示乱码。

**原因**：需要绘制的字符数据必须为 `GB2312` 编码。

**解决**：将字符转成 `GB2312` 编码后传入（如 `string.encode('gb2312')`）。

### chn 参数如何选择输出接口

**现象**：不清楚 `display` 的 `chn` 参数该填写多少。

**原因**：`chn` 参数已改为输出接口选择。

**解决**：`10` 表示 DP（DisplayPort）输出、`11` 表示 HDMI 输出，`0~3` 为旧值（默认按 HDMI 处理）。
