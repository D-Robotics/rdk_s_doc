...
RDK S100开发板提供一路HDMI显示接口（接口4）和一路DisplayPort接口(接口5)，最高支持1080P分辨率。开发板上电后会通过HDMI接口输出Ubuntu图形界面，配合特定的示例程序，同时还支持摄像头、视频流画面的预览显示功能。

目前接口只支持1080p60的显示模式，更多的显示模式会在后续的软件版本中支持。
...
- M.2 Key M：对应接口25，接口规格为 PCIe 3.0 x4，用于用于接入 UFS（通用闪存存储）设备。
