---
sidebar_position: 1
title: "开发者套件简介（RDK S100）"
description: 开发者套件简介（RDK S100）
...
<img src="https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/hardware_interface/image-rdk_100_mainboard_overview.png" alt="RDK S100 开发者套件" style={{ width: '100%', maxWidth: '980px', height: 'auto', display: 'block', margin: '0 auto' }} />
...
地瓜机器人 RDK S100 系列开发者套件搭载 S100 智能计算平台，BPU 可提供高达 80/128 TOPS 的算力，是一款面向智能计算与机器人应用的开发板，接口丰富，独特的异构设计可以同时兼顾感知推理和实时运动控制的需求，减少控制系统的体积和复杂度。
...
<img src="https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/hardware_interface/image-rdk_s100_architecture_diagram.png" alt="RDK S100 拓扑图" style={{ width: '100%', maxWidth: '980px', height: 'auto', display: 'block', margin: '0 auto' }} />

### 接口图

<img src="https://rdk-doc.oss-cn-beijing.aliyuncs.com/doc/img/01_Quick_start/image/hardware_interface/image-rdk_100_mainboard_interface.png" alt="RDK S100 接口图" style={{ width: '100%', maxWidth: '980px', height: 'auto', display: 'block', margin: '0 auto' }} />
...
| J8      | 风扇控制接口                                | J25     | 相机扩展板 100-Pin 接口 |
...
RDK S100 主板包含 RTC 外接电池连接器（J3），用于外接 RTC 备用电池，在主板掉电后保持实时时钟继续计时。板载 RTC 芯片为 YSN8130E，通过 I2C4 通信，I2C 地址为 `0x32`（不含读写位）。
...
1. 推荐使用 3V 电池，外接电池每次接入连接器时，电池电压建议在 2.5-3.5V，可充电电池耐压要求至少 3.3V。
2. 可充电电池可按需打开 RTC 充电功能；非充电电池禁止打开充电功能。
3. 系统正常上电时，RTC 优先由板端 VDD 供电；板端掉电后自动切换至电池（VBAT）供电。
4. 主板与 RTC（VBAT）均掉电后再次上电时，软件需做如下配置：
    - 通过 I2C 写入相关寄存器关闭 FOUT 功能，以防止电池供电时产生的额外功耗，影响电池续航能力。
    - 若使用可充电电池（如微雪 ML2032）需重新打开充电使能，非充电电池（如微雪 CR2032）禁止打开充电功能。
    - 若仅板端 VDD 掉电而 VRTC 仍保持供电，原有配置会保留，无需重新配置。
...
- USB 下载接口。RDK S100 开发板提供的一路下载接口用于固件下载，具体可以参考[系统烧录](../03_install_os_and_setup/01_instruction.md)
...
1. 当系统处于 light sleep 和 deep sleep 模式时，VDD_AON_PERI_MCU_3V3、VDD_AON_PERI_MCU_5V 电源保持供电，VDD_AON_PERI_MCU_3V3、VDD_AON_PERI_MCU_5V 其最大供电电流分别为 300mA 和 200mA。
...
3. 当系统处于 light sleep 和 deep sleep 模式时，VDD_PERI_3V3、VDD_PERI_12V，VDDIO_TOP_1V8 电源关闭，其中 3.3V 电源最大供电电流为 1700mA，12V 电源最大供电电流为 800mA。
...
2. 注意[USB 2.0 Camera 接入能力限制](/Demos/peripheral/camera/usb_camera#usb-2.0-note)。
   :::

## 开关、按键和灯光说明

### 按键 (K1/K2){#buttons-k1k2}
...
#### 系统启动盘选择 (SW3){#boot-device-selection-sw3}
...
本节介绍商业版硬件资料（产品手册和设计资料）的获取方式，公开资料参见下方[资料下载](#资料下载)节。
...
- 扩展板：[RDK S100 相机扩展板](./03_expansion_board/01_camera/01_rdk_camera_expansion_board.md)、[RDK S100 相机扩展板 12 通道](./03_expansion_board/01_camera/02_rdk_camera_expansion_board_12l.md)、[RDK S100 MCU 接口扩展板](./03_expansion_board/02_mcu/01_rdk_mcu_port_expansion_board.md)
- 烧录：[系统烧录](../03_install_os_and_setup/01_instruction.md)
- 硬件点亮：[RDK S100 硬件点亮](/Advanced_development/board_bringup/rdk_s100_bringup)
